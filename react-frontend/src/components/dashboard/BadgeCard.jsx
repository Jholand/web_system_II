...
const BadgeCard = ({ 
  ...
  return (
    <div className="group bg-white rounded-lg shadow-sm p-6 border border-slate-100 hover:shadow-md transition-all relative">
      ...
      {/* Description */}
      <p className="text-slate-600 text-sm mb-4 font-medium">{description}</p>

      {/* Unlock Criteria */}
      <div className="bg-slate-100 rounded-lg p-3 mb-4">
        <p className="text-slate-700 text-sm">
          <span className="font-bold">Unlock:</span> {unlockCriteria}
        </p>
      </div>

      {/* Users Earned */}
      <p className="text-slate-600 text-sm">
        <span className="font-bold text-slate-900">{usersEarned}</span> users have earned this
      </p>
    </div>
  );
};
