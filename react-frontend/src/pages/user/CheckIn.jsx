...
const CheckIn = React.memo(() => {
  ...
  const { data: checkinsData, isLoading: loadingCheckIns, refetch: refetchCheckins } = useCheckins(10);
  const { data: statsData, isLoading: loadingStats, refetch: refetchStats } = useCheckinStats();
  ...
  const stats = {
    ...
    all_time: statsData?.total_visits || 0
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-teal-50 via-cyan-50 to-blue-50 relative">
      ...
      <div className={`transition-all duration-300 ${(showReviewModal || showScanModal) ? 'blur-sm' : ''}`}>
      ...
      >
        <h2 
          className="text-2xl sm:text-3xl font-light tracking-tight text-gray-900 mb-6 sm:mb-8 animate-fade-in"
        >
          Your Adventure
        </h2>
        ...
        >
          ...
          >
            {/* QR Scanner Card */}
            <div 
              className="bg-white rounded-2xl p-6 sm:p-8 border border-emerald-200 text-center shadow-sm hover:shadow-lg transition-shadow duration-150 animate-fade-in"
            ...
            {/* Recent Check-Ins */}
            <div 
              className="bg-white rounded-2xl p-4 sm:p-6 border border-gray-200 shadow-sm hover:shadow-lg transition-shadow duration-150 animate-fade-in"
              ...
              <div className="space-y-3">
                {recentCheckIns.length > 0 ? (
                  recentCheckIns.map((checkIn, index) => (
                    <div key={checkIn.id || `checkin-${index}`} className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-3 sm:p-4 bg-gradient-to-r from-emerald-50 to-teal-50 rounded-lg border border-emerald-200 gap-3 sm:gap-0 hover:shadow-sm transition-shadow duration-75">
